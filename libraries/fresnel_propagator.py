...
class Fresnel_propagation(nn.Module):
    def __init__(self, **kwargs) -> None:
        ...
        self.phase = torch_reshape(self.phase)
        self.attenuation = torch_reshape(self.attenuation)
    ...
    def get_wavefield(self, phase = None, attenuation = None, downsampling_factor = 1, pad = None, mode = None, value = None):
        ...
        if mode == 'constant':
            ...
            if self.cut == None:
                wavefield_real = Fun.pad(wavefield.real, (int((shape_y*pad - shape_y)/2), int((shape_y *pad- shape_y)/2), int((shape_x *pad- shape_x)/2), int((shape_x *pad- shape_x)/2)), mode = mode, value = value_real)
                wavefield_imag = Fun.pad(wavefield.imag, (int((shape_y*pad - shape_y)/2), int((shape_y *pad- shape_y)/2), int((shape_x *pad- shape_x)/2), int((shape_x *pad- shape_x)/2)), mode = mode, value = value_imag)
            else:
            ...
            if pad is None:
                pad = 2
            if self.cut == None:
                wavefield = Fun.pad(wavefield, (int((shape_y*pad - shape_y)/2), int((shape_y *pad- shape_y)/2), int((shape_x *pad- shape_x)/2), int((shape_x *pad- shape_x)/2)), mode)
            else:
