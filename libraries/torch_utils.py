...
def prepare_dict(**kwargs):
    ...
    if kwargs['phase'] is not None:
        try:
            ...
            kwargs['phase'] = torch_reshape(kwargs['phase'].to_numpy())
            _, _, shape_x, shape_y = kwargs['phase'].shape
    ...
    if kwargs['attenuation'] is not None:
        try:
            ...
            kwargs['attenuation'] = torch_reshape(kwargs['attenuation'].to_numpy())
            _, _, shape_x, shape_y = kwargs['attenuation'].shape
...
def pos_neg_image(image, positive = None, reference_image = None, remove_extreme = False, retain_grad = False):
    ...
    if positive is False:
        ...
        image = torch.nn.functional.gelu(image)
...
def pad_multiple_times(im, n, pad_by = 2):
    ...
    for i in range(n):
        ...
        im = Fun.pad(im, (int((n*pad_by - n)/2), int((n *pad_by- n)/2), int((m *pad_by- m)/2), int((m *pad_by- m)/2),), mode = 'reflect')
    return im, shapes
...
def resize_with_diff_interpolation(img, size, interpolation = 'NEAREST_EXACT', out_type = 'np'):
    ...
    if interpolation == 'NEAREST' or 'nearest':
        new_image = (F.resize(img = (img), size = size, antialias=False, interpolation = InterpolationMode.NEAREST))
    elif interpolation == 'BOX' or 'box':
        new_image = (F.resize(img = (img), size = size, antialias=False, interpolation = InterpolationMode.BOX))
    elif interpolation == 'BILINEAR' or 'bilinear':
        new_image = (F.resize(img = (img), size = size, antialias=False, interpolation = InterpolationMode.BILINEAR))
    elif interpolation == 'HAMMING' or 'hamming':
        new_image = (F.resize(img = (img), size = size, antialias=False, interpolation = InterpolationMode.HAMMING))
    elif interpolation == 'BICUBIC' or 'bicubic':
        new_image = (F.resize(img = (img), size = size, antialias=False, interpolation = InterpolationMode.BICUBIC))
    elif interpolation == 'LANCZOS' or 'lanczos':
        new_image = (F.resize(img = (img), size = size, antialias=False, interpolation = InterpolationMode.LANCZOS))
    elif interpolation == 'NEAREST_EXACT' or 'nearest_exact':
        new_image = (F.resize(img = (img), size = size, antialias=False, interpolation = InterpolationMode.NEAREST_EXACT))
    if out_type == 'np':
